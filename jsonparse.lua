...
function parsejsoncontent(t, content)
    ...
    while s <= #content do
        ...
        if string.match(c, "^%s*%[") then
            ...
            local curr_array_data, error = findblock(string.sub(c, #blankspace), "[", "]")
            ...
            while s_cur <= #curr_array_data - 1 do
                ...
                if string.match(rest, "^%s*%[") then
                    ...
                    local subarraydata, errmsg = findblock(string.sub(rest, #blankspace2), "[", "]")

                    if subarraydata ~= false and errmsg == "" then
